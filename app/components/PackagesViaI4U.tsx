...
export default function PackagesViaI4U() {
  ...
  // טוען חבילות  ⬅️ עם AbortController
  useEffect(() => {
  ...
  // בדיקת שימוש בחבילה חינמית  ⬅️ עם AbortController
  useEffect(() => {
    const controller = new AbortController();

    (async () => {
      try {
        const res = await fetch("/api/payments/used-free", {
  ...
  // ⬇️ טעינת Billing Profile פעם אחת (ללא userId מהלקוח) — כבר עם AbortController
  useEffect(() => {
    const controller = new AbortController();
    (async () => {
      try {
