...
import { Trash2, Plus, Search, ChevronsUpDown, Send } from "lucide-react";
...
const GuestTable = ({ guestList }: GuestTableProps) => {
  ...
  const handleSendSMS = (guest: Guest) => {
    alert(`הודעת SMS נשלחת ל-${guest.name} (${guest.phone})`);
    // כאן תוכל להוסיף קריאה ל-API של Twilio
  };
  ...
  const uniqueTables = Array.from(new Set(guests.map((g) => g.table))).filter(
    Boolean
  );
  ...
  return (
    <div className="bg-white rounded-xl shadow p-6">
      ...
      <p className="text-sm text-center text-gray-600">
        סה"כ מוזמנים: {filteredGuests.length}
      </p>
      <p className="text-sm text-center text-green-700 mb-4">
        סה"כ צפויים להגיע: {totalComing}
      </p>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-right border">
            <thead className="bg-gray-100">
              <tr>
                {["phone", "name", "status", "table", "count"].map((col) => (
                  <th
                    key={col}
                    className="px-4 py-2 border cursor-pointer select-none"
                    onClick={() => handleSort(col as keyof Guest)}
                  >
                ...
                <th className="px-4 py-2 border">שלח SMS</th>
              </tr>
            </thead>
            <tbody>
                ...
                <tr key={index} className="odd:bg-white even:bg-gray-50">
                  ...
                  <td className="px-4 py-2 border">
                    <input
                      ...
                      value={guest.table}
                      onChange={(e) =>
                        handleChange(index, "table", e.target.value)
                      }
                      className="border rounded px-2 py-1 w-full"
                    />
                  </td>
