...
const GuestListManager = ({
    ...
    const fetchGuests = async () => {
      ...
      try {
        ...
        if (!Array.isArray(data)) {
          console.error("Guest API returned non-array data:", data);
          setGuestsList([]); // fallback to empty list
          setDisplayList([]);
          return;
        }

        // const normalized = data.map((guest: any) => ({
        //   ...guest,
        //   id: guest._id,
        // }));

        // data.map(({ guest._id, ...rest }) => ({ ...rest, id: _id }));

        console.log("fetch data Guests from DB :", data);
        // const normalized = data.map((guest: any) => {
        //   const { _id, ...rest } = guest;
        //   return { ...rest, id: _id };
        // });
        console.log("fetch normalize with id :", data);

        setGuestsList(data);
        console.log("guestsList from db:", guestsList);
        setDisplayList(data);
      } catch (error) {
  ...
  const updateGuests = async (newGuests: Guest[]) => {
    ...
    if (session?.user?.email) {
      await fetch("/api/guests/bulk", {
        ...
        body: JSON.stringify({
          guests: newGuests, // already includes _id
          ownerEmail: session.user.email,
          eventId: eventDetails._id, // ← זהו השדה החדש שצריך להעביר
        }),
      });
    }
    ...
    if (hasErrors) {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          console.log("✅ Tables saved:", savedTables);

          const sortedTables = savedTables.sort(
            (a: Table, b: Table) => parseInt(a.number) - parseInt(b.number)
          );

          setTables(sortedTables); // מעדכן את ה־state של הטבלאות
        }
      } catch (err) {
  ...
  const loadTables = async (email: string) => {
    ...
    setTables(data); // ⬅️ אתה כבר צריך שיהיה useState לטבלאות
  };
  ...
  const saveTablesToDB = async (tables: Table[], email: string) => {
    if (!email) return;
    console.log("==saveTablesToDB== :", tables);
