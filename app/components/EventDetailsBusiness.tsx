...
const EventDetailsBusiness = ({
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-1 gap-y-1 max-w-md w-full px-2 text-right">
      <InputBox1
        name="brideFirst"
        titleName={`שם האירוע${mark("brideFirst")}`}
        value={details.brideFirst}
        onChange={onChange}
        showError={isRequired?.("brideFirst") && !details.brideFirst}
        placeholder="לדוגמה: כנס אינטרנט"
      />

      <InputBox1
        name="brideLast"
        titleName={`בעל הארוע${mark("brideLast")}`}
        value={details.brideLast}
        onChange={onChange}
        placeholder="לדוגמה: כיוונים"
...
export default EventDetailsBusiness;
