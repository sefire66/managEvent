...
const GuestStats = ({ guestList, filters, setFilters }: GuestStatsProps) => {
  ...
  const totalCount = guestList.reduce(
    (total, guest) => total + Number(guest.count || 0),
    0
  );
  const coming = guestList.filter((g) => g.status === "בא").length;
  const maybe = guestList.filter((g) => g.status === "אולי בא").length;
  const notComing = guestList.filter((g) => g.status === "לא בא").length;
  const noReply = guestList.filter((g) => g.status === "לא ענה").length;
  ...
  return (
    <div className="space-y-4">
      <div className="bg-white rounded-xl shadow p-6 text-center grid grid-cols-2 sm:grid-cols-4 gap-4">
        <StatBox label="סהכ הודעות לאורחים בכל משלוח" value={total} />
        <StatBox label="סהכ מוזמנים" value={totalCount} />
