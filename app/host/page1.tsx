...
type Guest = {
  ...
  table?: string | null;
  arrived?: boolean;
  count?: number; // ✅ how many people come with this guest
};
...
// נרמול סוג אירוע (לתקן "עיסקי" ל-"עסקי")
...
// בניית טקסט כותרת כמו ב-EventTitle (ללא אימוג'ים)
...
export default function HostDashboard() {
    ...
    const fetchGuests = async (email: string, evId: string) => {
      try {
        ...
        const normalized: Guest[] = data.map((g: any) => ({
          ...
          count: typeof g.count === "number" ? g.count : 1, // ✅ default 1
        }));
  ...
  // Header text (built from eventDetails)
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              {eventError ? (
                ...
                <p className="hidden sm:block text-xs sm:text-xs text-white">
                  {loadingEvent ? "" : headerSubtitle}
                </p>
              )}

              <p className="hidden sm:block text-xs text-white mt-1">
                (מארח של {clientEmail})
              </p>
            </div>
          </div>
